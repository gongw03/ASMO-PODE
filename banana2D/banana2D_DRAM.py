...
[Chain, LogPost, ACC, GRB] = \
    DRAM.sampler(model, D, xlb, xub, None, None, T, B, N, M, \
        parallel)
...
plt.ylabel('x2')
# save figure
plt.savefig('%s/banana2D_DRAM.png' % respath)

# save results to bin file
# with open('%s/banana2D_DRAM.bin' % respath,'w') as f:
#     cPickle.dump({'D':D, 'T': 1, 'B': B, 'N': N, 'M': M, \
#                   'Chain': Chain, 'LogPost': LogPost, \
#                   'ACC': ACC, 'GRB': GRB}, f)
with open('%s/banana2D_DRAM.bin' % respath,'wb') as f:
