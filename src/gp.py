...
from sklearn.gaussian_process.kernels import (RBF, Matern, RationalQuadratic, \
        ExpSineSquared, DotProduct, ConstantKernel, WhiteKernel)
import copy


class GPR_Matern:
    def __init__(self, xin, yin, nInput, nOutput, N, xlb, xub, boxcox = None):
        # boxcox is designed for skewed data, such as -2loglikelihood in ASMO-PODE
        # but the application result is not good, do not use
        # boxcox = None: no Box-Cox transformation
        # boxcox = [0,1]: this is suggested for MCMC
        self.nInput  = nInput
...
#        kernel = 1.0 * Matern(length_scale=1.0, length_scale_bounds=(1e-3, 1e+3), nu=2.5) \
#                + WhiteKernel(noise_level=1, noise_level_bounds=(1e-3, 1e+3))
        #kernel = ConstantKernel(constant_value=1.0, constant_value_bounds=(1e-3, 1e+3)) * \
        #         Matern(length_scale=1.0, length_scale_bounds=(1e-3, 1e+3), nu=2.5) + \
        #         WhiteKernel(noise_level=1.0, noise_level_bounds=(1e-3, 1e+3))

        # good!
        #kernel = ConstantKernel(constant_value=1.0, constant_value_bounds=(1, 10000)) * \
        #         Matern(length_scale=1.0, length_scale_bounds=(1, 10000), nu=2.5)
        # try
        kernel = ConstantKernel(constant_value=1.0, constant_value_bounds=(1e-6, 1e6)) * \
                 Matern(length_scale=1.0, length_scale_bounds=(1e-6, 1e6), nu=2.5)

        smlist = []
        for i in range(nOutput):
            smlist.append(GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=5))
            #smlist.append(GaussianProcessRegressor(kernel=kernel, alpha=1e-5, optimizer=sceua_optimizer))
            smlist[i].fit(x,y[:,i])
            print(smlist[i].kernel_)
        self.smlist = smlist
